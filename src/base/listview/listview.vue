<template>
  <m-scroll class="listview"
            ...
            @scroll="scroll">
    <ul>
      <li v-for="group in singersdata" class="list-group" ref="listGroup">
    ...
    <div class="list-shortcut" @touchstart="onShortcutTouchStart($event)"
      ...
      <ul>
        <li :data-index="index" :class="{'current':currentIndex===index}" v-for="(item,index) in shortcutList"
            class="item">{{item}}
        </li>
      </ul>
    </div>
  ...
  export default {
    ...
    computed: {
      ...
      fixedTitle() {
        ...
        return this.singersdata[this.currentIndex] ? this.singersdata[this.currentIndex].title : "";
      }
    },
    watch: {
      singersdata() {
